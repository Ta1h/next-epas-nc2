"use client";
import { ChevronLeft, FileEdit, FileX2, Plus } from 'lucide-react';
...
const page: FC<props> = ({params}) => {
  ...
  return (
    <div className='p-10'>
      	<Link href={'/adminDashboard/lessons'} className='flex text-sm text-gray-500 '>
			<ChevronLeft className='h-5'/>
			<h1>Lessons</h1>
		</Link>
		<div className='flex'>
			<h1 className='font-semibold w-full text-xl pl-2 pb-10'>{unitNumber} {unitTitle}</h1>
      	...
      	<div className='grid justify-center items-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7'>
			{selectedLesson ? (
				unitZero === selectedLesson.unitId ? (
					<Link href={''} className='flex-col p-5 h-40 rounded-md hover:bg-gray-50 shadow-[0px_3px_8px_0px_#00000024]'>
						<div>
