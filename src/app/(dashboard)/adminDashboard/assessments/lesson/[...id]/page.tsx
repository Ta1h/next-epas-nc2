...
const page: FC<props> = ({params}) => {
  ...
  return (
    <div className='p-10'>
      <Link href={'/adminDashboard/assessments/unit/'+lessonId[3]+'/'+lessonId[4]+'/'+lessonId[5]} className='flex text-sm text-gray-500 '>
        <ChevronLeft className='h-5'/>
        <h1>back</h1>
      </Link>
      <div className='flex'>
        <h1 className='font-semibold w-full text-xl pl-2 pb-10'>{lessonNumber}{lessonTitle}</h1>
              ...
              <div className='flex-col w-full space-y-3 p-5 rounded-md shadow-[0px_3px_8px_0px_#00000024]'>
                ...
                <div>
                  {route.choices.map((choice, index) => (
                    <ul key={choice.id}>
                      <li className={`pl-7 ${route.correctAnswer.choiceId === choice.id ? 'font-bold text-green-500' : ''}`}> 
